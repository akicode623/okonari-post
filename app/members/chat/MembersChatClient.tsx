...
export default function MembersChatClient({
  initialMessages,
}: {
  ...
  useEffect(() => {
    const savedName = window.localStorage.getItem("members-chat-display-name");
  ...
  useEffect(() => {
    if (trimmedName) {
      window.localStorage.setItem("members-chat-display-name", trimmedName);
    }
  }, [trimmedName]);
  ...
  async function refreshMessages(showError = true) {
    try {
      ...
      const res = await fetch("/api/chat/messages?take=100", {
        method: "GET",
        cache: "no-store",
      });
      ...
      if (showError) {
        setErrorText("メッセージの再取得に失敗しました。時間をおいて再試行してください。");
      }
    } finally {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setErrorText("メッセージ送信に失敗しました。内容を確認して再試行してください。");
    } finally {
      setIsSending(false);
    }
  }
      ...
      <aside className="rounded-2xl border border-[#1b1c18]/10 bg-[#fbfaf6] p-5">
        <div className="space-y-3">
          ...
          <p className="text-xs leading-relaxed text-[#3f4137]">
            会員同士の交流スペースです。個人情報は投稿せず、互いに配慮して利用してください。
          </p>
        </div>
      </aside>

      <section className="rounded-2xl border border-[#1b1c18]/10 bg-[#fbfaf6] p-5">
        <div className="h-[440px] overflow-y-auto rounded-xl border border-[#1b1c18]/10 bg-white px-4 py-4">
          <div className="space-y-3">
            {messages.length === 0 && (
                ...
                >
                  ...
                  >
                    <div className="mb-1 flex items-center gap-2 text-xs opacity-80">
                      ...
                      <time>{new Date(m.createdAt).toLocaleString("ja-JP")}</time>
                    </div>
        ...
        <form onSubmit={onSubmit} className="mt-4 space-y-3">
          ...
          <div className="flex flex-col items-start justify-between gap-2 sm:flex-row sm:items-center">
            <p className="text-xs text-[#5c5f55]">{messageInput.length}/500 文字</p>
            <button
              type="submit"
              disabled={!canSubmit}
